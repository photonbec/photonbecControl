...
if gethostname() == "ph-photonbec5":
    ...
    camera_label = 'blackfly_semiconductor_cavity_lock'
    hardwidth = 800
    hardheight = 800  # image size for flea
    import SingleChannelAO
            ...
            while self.running:
                ...
                while not self.paused:
                    ...
                    if parent.initial_analysis is False:
                        ...
                        images = list()
                        parent.trial_voltages = np.linspace(parent.control_range[0], parent.control_range[1],
                                                            parent.number_images_for_PCA)
                        ...
                        pca_components_2D = np.reshape(pca_components_flattened,
                                                       [pca_components_flattened.shape[0], images_shape[1],
                                                        images_shape[2]])
                        ...
                        normalization_factor = np.max(
                            [np.abs([np.min(projections), np.max(projections)])]) / parent.error_signal_amplitude
                        ...
                        matplotlib.image.imsave('lock_image_main_PCA_component.jpg',
                                                np.squeeze(pca_components_2D[0, :, :]))
                    ...
                    try:
                        ...
                        print('Frame time = {0} ms / Analysis time = {1} ms / Lock rate = {2} Hz'.format(
                            int(time2 - time1), int(time3 - time2), locking_rate))
