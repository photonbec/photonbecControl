...
class FianiumLaser():
	...
	def __init__(self, comport=fianium_comport, wait_time=10e-2):
		if type(comport) == int:
			self.ser = serial.Serial('COM4', baudrate=19200, timeout=0.2)
		else:
	...
	def writeCommand(self, s, wait_time=10e-2):
		...
		self.ser.write(str.encode(s + line_end))
	...
	def read(self, size=None):
		# self.open()
		if size != None:
			res = self.ser.read(size).decode("utf-8")
		else:
			res = self.ser.readall().decode("utf-8")
		# self.close()
		return res
	...
	def findPulseEnergyParameters2(self, pulse_energy):
		if 0.1 <= pulse_energy < 0.6:
			cv = 0
			coeffs = [1.94459015e-12, -1.81483740e-09, 5.17304397e-07, -4.00311154e-05, 8.31747187e-04,
					  -1.65116819e-03 - pulse_energy]
			roots = poly.polyroots(list(reversed(coeffs)))
			out_amp = max([i.real for i in roots if 0 <= i <= 255])
		elif 0.6 <= pulse_energy < 6:
			cv = 150
			coeffs = [1.41450562e-11, -1.56935466e-08, 4.84624652e-06, -4.02332700e-04, 1.05627054e-02,
					  -4.51490621e-02 - pulse_energy]
			roots = poly.polyroots(list(reversed(coeffs)))
			out_amp = max([i.real for i in roots if 0 <= i <= 255])
		elif 6 <= pulse_energy < 50:
			cv = 200
			coeffs = [1.04214979e-10, -1.37340226e-07, 4.41423797e-05, -3.46540682e-03, 8.62171737e-02,
					  -3.39443464e-01 - pulse_energy]
			roots = poly.polyroots(list(reversed(coeffs)))
			out_amp = max([i.real for i in roots if 0 <= i <= 255])
		elif 50 <= pulse_energy < 300:
			cv = 300
			coeffs = [2.61473188e-09, -2.04138516e-06, 5.04291260e-04, -3.57951343e-02, 8.25651766e-01,
					  -2.91489951e+00 - pulse_energy]
			roots = poly.polyroots(list(reversed(coeffs)))
			out_amp = max([i.real for i in roots if 0 <= i <= 255])
		# print coeffs
		elif 300 <= pulse_energy < 800:
			cv = 400
			coeffs = [6.45286337e-09, -4.74508305e-06, 1.09800111e-03, -6.98114957e-02, 1.43770885e+00,
					  -4.39976880e+00 - pulse_energy]
			roots = poly.polyroots(list(reversed(coeffs)))
			out_amp = max([i.real for i in roots if 0 <= i <= 255])
		elif 800 <= pulse_energy < 1500:
			cv = 600
			coeffs = [1.72181502e-08, -1.13030471e-05, 2.27310235e-03, -1.06494515e-01, 1.33015298e+00,
					  -1.59100539e-01 - pulse_energy]
			roots = poly.polyroots(list(reversed(coeffs)))
			out_amp = max([i.real for i in roots if 0 <= i <= 255])
		elif 1500 <= pulse_energy <= 2500:
			cv = 800
			coeffs = [1.29984873e-08, -8.13153864e-06, 1.36863062e-03, 1.43969378e-02, -2.62275026e+00,
					  1.83899589e+01 - pulse_energy]
			roots = poly.polyroots(list(reversed(coeffs)))
			out_amp = max([i.real for i in roots if 0 <= i <= 255])
		else:
			print('Error: Pulse Energy cannot be read.')
